...
class _LoginPageState extends State<LoginPage> {
  ...
  void _navigateToController(String email) {
    if (mounted) {
  ...
  void _handleLogin() async {
    if (_usernameController.text.isEmpty || _passwordController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Masukkan email dan password'),
    ...
    try {
      final credential = await AuthService().loginWithVerificationCheck(
        _usernameController.text.trim(),
        _passwordController.text,
      );

      final email = credential.user?.email ?? _usernameController.text.trim();
      _navigateToController(email);
    } on UnverifiedEmailException catch (e) {
      // Email belum diverifikasi → arahkan ke halaman verifikasi
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
      ...
      if (mounted) {
        ...
        switch (e.code) {
          ...
          case 'too-many-requests':
            displayMessage = 'Terlalu banyak percobaan. Coba lagi nanti.';
            break;
          case 'invalid-email':
            displayMessage = 'Format email tidak valid.';
            break;
          default:
            displayMessage = e.message ?? 'Login gagal';
        }
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            OutlinedButton(
                              onPressed:
                                  anyLoading ? null : _handleGoogleSignIn,
  ...
  Widget _buildTextField(TextEditingController ctrl, String hint,
    ...
    return TextField(
      ...
      onSubmitted:
          isPassword ? (_) => _isLoading ? null : _handleLogin() : null,
    );
  }
}
